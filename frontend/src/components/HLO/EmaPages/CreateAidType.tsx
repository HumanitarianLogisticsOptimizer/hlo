...
const CreateAidType: React.FC = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = 'http://24.133.52.46:8000/api/aid_type/';
    ...
    try {
      ...
      setName('');
      setMessage('Aid type created successfully.');
    } catch (error) {
      console.error('Error creating aid type: ', error);
      setMessage('Error creating aid type.');
    } finally {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Create New Aid Type" />
      {message && (
