...
const ManageCenters: React.FC = () => {
  ...
  const data = useMemo(() => openTab === 1 ? accs : adcs, [accs, adcs, openTab]);
        ...
        Cell: ({ row: { original } }) => (
          <div className="flex gap-2">
            <button
              ...
              onClick={() => navigate(`/hlo/admin/update-${openTab === 1 ? 'acc' : 'adc'}/${original.id}`)}
            >
              Update
            </button>
            <button
              className="inline-flex items-center justify-center rounded-md border border-primary px-4 text-center font-medium text-primary dark:border-white dark:text-white hover:shadow-4 lg:px-8 xl:px-10"
              onClick={() => handleDelete(original.id, openTab === 1 ? 'acc' : 'adc')}
            >
              Delete
            </button>
          </div>
            ...
            {page.map((row, key) => {
              ...
              return (
                <tr {...row.getRowProps()} key={key}>
                  ...
                  <td className=' gap-3'>
                    <button
                      className="inline-flex p-1 items-center justify-center rounded-md border border-primary px-4 text-center font-medium text-primary dark:border-white dark:text-white hover:shadow-4 lg:px-8 xl:px-10"
                      onClick={() => navigate(`/hlo/admin/update-${openTab === 1 ? 'acc' : 'adc'}/${row.original.id}`)}
                    >
                      Update
                    </button>
                    <button
                      className="inline-flex p-1 items-center justify-center rounded-md border border-primary px-4 text-center font-medium text-primary dark:border-white dark:text-white hover:shadow-4 lg:px-8 xl:px-10"
                      onClick={() => handleDelete(row.original.id, openTab === 1 ? 'acc' : 'adc')}
                    >
                      Delete
                    </button>
                  </td>
